...
void insertData(HashTable *table, void *key, void *data) {
  ...
  if(table->data[loc] == NULL) {
    table->data[loc] = (struct HashBucket *) malloc(sizeof(struct HashBucket));
    struct HashBucket *it = table->data[loc];
    it->key = key; it->data = data; it->next = NULL;
  }
  else {
    struct HashBucket *it = table->data[loc];
    while(it->next) {
    ...
    it->next = (struct HashBucket *) malloc(sizeof(struct HashBucket));
    it = it->next;
    it->key = key; it->data = data; it->next = NULL;
  }
}
...
void *findData(HashTable *table, void *key) {
  ...
  struct HashBucket *it = table->data[loc];
  while(it) {
    if(table->equalFunction(it->key, key)) {
	    return it;
    }
    it = it->next;
  }
